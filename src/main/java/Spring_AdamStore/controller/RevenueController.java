...
public class RevenueController {
    ...
    public ApiResponse<List<OrderRevenueDTO>> getOrderRevenueByDate(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
                                                                        @Parameter(example = "2025-02-20") LocalDate startDate,
                                                                    @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
                                                                        @Parameter(example = "2025-05-10") LocalDate endDate){
        return ApiResponse.<List<OrderRevenueDTO>>builder()
                ...
                .result(revenueService.getOrderRevenueByDate(startDate, endDate))
                .build();
    }
}
